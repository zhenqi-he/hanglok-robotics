...
def main():
    ...
    SAVE_DIR = r'C:\Users\HP\Desktop\hzq\data_testing'
    create_dir(SAVE_DIR)
    ...
    saved_img_name = os.path.join(SAVE_DIR,str(0)+'.png')
    ...
    for i in range(2):
        ...
        for s in range(step):
            ...
            # along the signed axis move a certain length
            if 1:
                current_pnt[i] += step_length
            # else:
            #     current_pnt[i] -= step_length
            str_pnt = ''    
            for c in range(6):
                str_pnt += ','+str(current_pnt[c])
            f.write(str_pnt+'\n')
