...
def main():
    ...
    R_camera2gripper, T_camera2gripper = cv2.calibrateHandEye(R_gripper2base_lists_train, T_gripper2base_lists_train,
                                                            R_target2camera_lists_train, T_target2camera_lists_train,method=cv2.CALIB_HAND_EYE_ANDREFF)
    ...
    for i in range(len(R_gripper2base_lists_test)):
        M_target2camera = np.row_stack((np.column_stack((R_target2camera_lists_test[i],0.1*T_target2camera_lists_test[i])),np.array([0,0,0,1])))
        M_camera2gripper = np.row_stack((np.column_stack((R_camera2gripper,0.1*T_camera2gripper)),np.array([0,0,0,1])))
        M_gripper2base = np.row_stack((np.column_stack((R_gripper2base_lists_test[i],0.1*T_gripper2base_lists_test[i])),np.array([0,0,0,1])))
...
# {"T_1": 125.33595784162009, "R_11": 0.9838000592964286, "R_12": 0.09882801230101096, "R_13": 0.1495676011473565, "T_2": 1250.0794244066863, "R_21": -0.0667969211120365, "R_22": 0.9763226850150545, "R_23": -0.20574787011035364, "T_3": -298.69668167324096, "R_31": -0.16635989498162068, "R_32": 0.1924241115599233, "R_33": 0.967107722351589}
